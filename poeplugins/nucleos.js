let nucleos = function (par) {
  ...
  let jsonfile = `https://docs.google.com/spreadsheets/d/1lncqczCIBX0fl9O0XpDt_ogiqQW5iZUKX-kqH6yHNVQ/edit#gid=1858811885`;
    ...
    getcsvdata(GoogleSheetCsvURL(jsonfile), function (dados) {
        ...
        let registroscat = unique(newarr, "nucleo");
        let separaregistros = [];
        let z = 0;
        for (let i = 0; i < registroscat.length; i++) {
            if (typeof registroscat[i] != "undefined" && registroscat[i] != "undefined" && registroscat[i] != "" && registroscat[i] != null) {
                ...
                separaregistros[z] = {};
                separaregistros[z].nucleo = registroscat[i];
                z++;
            }
        }

        let categorias = tags(separaregistros, "nucleo", ",");
        ...
        for (let c = 0; c < categorias.length; c++) {
            ...
            for (let i = 0; i < newarr.length; i++) {
                ...
                if (typeof newarr[i].nucleo != 'undefined' && newarr[i].nucleo != 'undefined' && newarr[i].nucleo != null && newarr[i].nucleo != '') {
                   
                    patt = String(newarr[i].nucleo);
